...
function initializeMenu(menuData, options = {}) {
  ...
  const focusSentinelStart = document.getElementById(
    'miller-focus-sentinel-start',
  );
  ...
  function getMenuFocusableElements({ includeSentinels = false } = {}) {
    ...
    return Array.from(elements).filter((el) => {
      ...
      if (
        !includeSentinels &&
        (el === focusSentinelStart || el === focusSentinelEnd)
      )
        return false;
      return true;
    });
  ...
  function setRovingTabIndex(columnEl, preferredId) {
    ...
    const preferred =
      (preferredId &&
        columnEl.querySelector(`.column-item[data-id="${preferredId}"]`)) ||
      items[0];
    preferred.setAttribute('tabindex', '0');
  }

  function focusColumnItem(level, preferredId) {
    const column = columnsWrapper.querySelector(
      `.column[data-level="${level}"]`,
    );
    if (!column) return;
    ...
    if (
      ...
      columnsWrapper instanceof HTMLElement
    ) {
  ...
  function updateMainMenuAria() {
    mainMenuItems.forEach((mi) => {
      ...
      const expanded = Boolean(
        key &&
        activeMainMenu === key &&
        columnMenuContainer.classList.contains('active'),
      );
  ...
  function addColumn(columnData, level) {
    ...
    columnData.items.forEach((item) => {
      ...
      if (item.hasChildren) {
        ...
        itemEl.setAttribute(
          'aria-expanded',
          activePath[level] === item.id ? 'true' : 'false',
        );
      }
  ...
  function handleItemClick(item, level, { isLink = false } = {}) {
    ...
    const currentItems = columnsWrapper.querySelectorAll(
      '.column-item.current',
    );
    ...
    if (columns[level]) {
      ...
      const clickedItem = columns[level].querySelector(
        `[data-id="${item.id}"]`,
      );
      if (clickedItem) {
        ...
        const active = columns[level].querySelector(
          '.column-item[tabindex="0"]',
        );
